...
document.addEventListener('DOMContentLoaded',function(){
    ...
    fetch('http://localhost/HW1/source/profile/PHPs/loadOrders.php')
    .then(response => response.json() )
    .then( datas=>{
        
        console.log(datas);
        const parentDiv=document.querySelector('#listing');

        if(datas.length == 0){

            document.querySelector('#no-order').textContent="No orders placed yet";
